...
BUILD_DIR=./build
BIN_DIR=./bin
GCC_FLAGS=-ffreestanding -m32 -O2 -nostdlib -Wall -Werror -fno-stack-protector
GCC=i686-elf-gcc
LD=i686-elf-ld
...
pack_custom_boot:
	...
	nasm -f bin $(SRC_DIR)/boot/bootloader.asm -o $(BUILD_DIR)/custom_bootloader/boot.bin
	nasm -f elf $(SRC_DIR)/boot/kernel_entry.asm -o $(BUILD_DIR)/custom_bootloader/kernel_entry.o
	$(LD) --oformat binary -T$(SRC_DIR)/linker.ld $(BUILD_DIR)/custom_bootloader/kernel_entry.o $(BUILD_DIR)/common/kernel.o -o $(BUILD_DIR)/custom_bootloader/kernel.bin
	mkdir -p $(BIN_DIR)
...
pack_multiboot:
	...
	nasm -f elf $(SRC_DIR)/boot/multiboot.asm -o $(BUILD_DIR)/multiboot/multiboot.o
	mkdir -p $(BUILD_DIR)/multiboot/iso/boot/grub
	$(LD) -T$(SRC_DIR)/linker.ld $(BUILD_DIR)/multiboot/multiboot.o $(BUILD_DIR)/common/kernel.o -o $(BUILD_DIR)/multiboot/iso/boot/mn1os.bin
